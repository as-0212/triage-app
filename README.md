...
This Java app models a triage system designed for an Earthquake Disaster Scenario. Here is the
...
- When patient admissions are done, the system will sort patients by injury level
  and demographic; patients with injury level 5 are considered to be in critical
  condition which are put into the Priority Queue and sorted by age demographic
  as follows: Children > Elders > Adults.
...
- Patient C: Child with injury level 5
...
**FIFO Test**  
Description: Demonstrates priority system when two patients are admitted that are not in critical 
condition and of the same age demographic.
...
the project, which I think can be considered a human error on our part for not specifying that in 
the first prompt.
